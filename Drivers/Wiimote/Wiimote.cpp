...
{
	wiiKeys.nX = data[1];
	if (data[1] == 0x32) // Nunchuck data
	{
		// http://wiibrew.org/wiki/Wiimote/Extension_Controllers/Nunchuck
		uint8_t* ext = data + 4;
		//printf("Nunchuk: sx=%3d sy=%3d c=%d z=%d\n", ext[0], ext[1], ext[5],ext[5]);
		wiiKeys.nunDataAvailable = true;
		wiiKeys.nX = ext[0];
		wiiKeys.nY = ext[1];
		//wiiKeys.nC = (0 == (ext[5] & 0x02));
		//wiiKeys.nZ = (0 == (ext[5] & 0x01));
		wiiKeys.nC = !(0x02 == (ext[5] & 0x02));
		wiiKeys.nZ = !(0x01 == (ext[5] & 0x01));
	}
	else
	{
		wiiKeys.nunDataAvailable = false;
	}
	wiiKeys.b1 = ((data[3] & 0x02) == 0x02);
	wiiKeys.b2 = ((data[3] & 0x01) == 0x01);
	wiiKeys.bA = ((data[3] & 0x08) == 0x08);
	wiiKeys.bB = ((data[3] & 0x04) == 0x04);
	wiiKeys.bPlus = ((data[2] & 0x10) == 0x10);
	wiiKeys.bMinus = ((data[3] & 0x10) == 0x10);
	wiiKeys.bHome = ((data[3] & 0x80) == 0x80);
	wiiKeys.aLeft = ((data[2] & 0x01) == 0x01);
	wiiKeys.aRight = ((data[2] & 0x02) == 0x02);
	wiiKeys.aDown = ((data[2] & 0x04) == 0x04);
	wiiKeys.aUp = ((data[2] & 0x08) == 0x08);
}
...
static esp_err_t _queue_data(xQueueHandle queue, uint8_t *data, size_t len) 
{
	...
	if (usePsram)
	{
		lendata = (lendata_t*)ps_malloc(sizeof(lendata_t) + len);
	}
	else
	{
		lendata = (lendata_t*)malloc(sizeof(lendata_t) + len);
	}	
	if (!lendata) {
...
void Wiimote::init(bool usePSRAM)
{
	if (usePSRAM)
	{
		usePsram = true;
	...
	//register_callback(1, wiimoteCallback);
	wiimote_callback[0] = wiimoteCallback;
}
...
bool Wiimote::readKeys(WII_KEYS * wiiKeysP)
{
	//Wiimote::handle();
	if (!wiimoteConnected)
	{
		...
		wiiKeysP->bA = false;
		wiiKeysP->bB = false;
		wiiKeysP->bPlus = false;
		wiiKeysP->bMinus = false;
		wiiKeysP->bHome = false;
		wiiKeysP->aLeft = false;
		wiiKeysP->aRight = false;
		wiiKeysP->aDown = false;
		wiiKeysP->aUp = false;

		return false;
	}

	wiiKeysP->b1 = wiiKeys.b1;
	wiiKeysP->b2 = wiiKeys.b2;
	wiiKeysP->bA = wiiKeys.bA;
	wiiKeysP->bB = wiiKeys.bB;
	wiiKeysP->bPlus = wiiKeys.bPlus;
	wiiKeysP->bMinus = wiiKeys.bMinus;
	wiiKeysP->bHome = wiiKeys.bHome;
	wiiKeysP->aLeft = wiiKeys.aLeft;
	wiiKeysP->aRight = wiiKeys.aRight;
	wiiKeysP->aDown = wiiKeys.aDown;
	wiiKeysP->aUp = wiiKeys.aUp;
	wiiKeysP->nunDataAvailable = wiiKeys.nunDataAvailable;
	wiiKeysP->nX = wiiKeys.nX;
	wiiKeysP->nY = wiiKeys.nY;
	wiiKeysP->nC = wiiKeys.nC;
	wiiKeysP->nZ = wiiKeys.nZ;
