...
DateTime::DateTime (const char* date, const char* time) {
	...
	{
		mm = conv2d(time + index);
		index += 3;
	}

	if ((index + 1) == strlen(time))
	{
		ss = time[index] - '0';
	}
	else
	{
		ss = conv2d(time + index);
	}
	//ss = conv2d(time + 6);
}
...
DateTime RTC_DS1307::nowFast()
{
	...
	Wire.endTransmission();*/
	buffer[0] = 0;
	Wire.writeTransmission(DS1307_ADDRESS, buffer, 1);
	Wire.readTransmission(DS1307_ADDRESS, buffer, 7);
...
uint8_t RTC_DS1307::readSec()
{
	...
	Wire.requestFrom((uint8_t)DS1307_ADDRESS, 1);
	temp = Wire._I2C_READ() & 0x7f;
	return bcd2bin(temp);
}

void RTC_DS1307::writeSec(uint8_t newSec)
{
	Wire.beginTransmission(DS1307_ADDRESS);
	Wire._I2C_WRITE(0);
	
	Wire._I2C_WRITE(bin2bcd(newSec));
	Wire.endTransmission();
}
